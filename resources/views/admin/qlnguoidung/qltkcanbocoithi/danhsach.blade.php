...
@section('pagetitle')
Quản lý tài khoản cán bộ coi thi
@endsection
@section('content')
		  ...
		  <table class="table datatable table-hover table-sm">
		  	<thead>
				<tr>
					...
					<th class="small" width="13%">Mã cán bộ</th>
					...
					<tr>
						...
						<td>
							@if($value->trangthai==1)
								...
								<i class="bi bi-toggle2-on text-primary" title="Đang kích hoạt"></i>
								</a>
								</h2>
							@else
								<h2><a href="#kichhoat" data-bs-toggle="modal" data-bs-target="#myModalKichHoat" onclick="getKichHoat({{ $value->id}},{{$value->trangthai}},'{{$value->name}}'); return false;">
									<i class="bi bi-toggle2-off text-danger" title="Đang khoá"></i>
									</a>
								</h2>
							@endif
						</td>
						...
						{{-- <td class="text-center small"><a class="btn btn-danger btn-sm" data-bs-toggle="modal" data-bs-target="#myModalDelete" onclick="getXoa({{ $value->id}}); return false;" ><i class="fa-regular fa-trash-can"></i></a></td>
					 --}}
					</tr>
				@endforeach
			</tbody>
...
<form action="{{ route('admin.qlnguoidung.qltkcanbocoithi.nhap') }}" method="post" enctype="multipart/form-data">
  ...
  <form action="{{route('admin.qlnguoidung.qltkcanbocoithi.them')}}" method="post">
	...
	<form action="{{ route('admin.qlnguoidung.qltkcanbocoithi.trangthai')}}" method="post">
		@csrf
	...
	<form action="{{ route('admin.qlnguoidung.qltkcanbocoithi.xoa') }}" method="post">
		@csrf
		...
		<div class="modal fade" id="myModalDelete" tabindex="-1" role="dialog" aria-labelledby="myModalLabelDelete">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-footer">
						<button type="button" class="btn btn-secondary" data-bs-dismiss="modal"> Hủy bỏ</button>
						<button type="submit" class="btn btn-danger"></i> Thực hiện</button>
					</div>
				</div>
			</div>
		</div>
